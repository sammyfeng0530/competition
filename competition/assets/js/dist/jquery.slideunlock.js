...
function SliderUnlock(elm, options, success, always) {
    ...
    var opts = {
        labelTip: typeof(options.labelTip)!=="undefined" ? options.labelTip : "向右滑动验证",
        successLabelTip: typeof(options.successLabelTip)!=='undefined' ? options.successLabelTip : "成功验证",
...
SliderUnlock.prototype.updateView = function () {
  var _self = this;
    ...
    if (_self.index == (_self.max - 0)) {
        ...
        _self.elm.find("i").removeClass("fa-angle-double-right").addClass("fa-check-circle");
    } else {
        $(".slideunlock-lockable").val(0);
        _self.elm.removeClass("success").find(".slideunlock-lable-tip").html(_self.opts.labelTip);
        _self.elm.find("i").removeClass("fa-check-circle").addClass("fa-angle-double-right");
    }
    _self.always();
};
